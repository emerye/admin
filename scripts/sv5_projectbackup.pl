...
#  12-17-16 project directory only 
###################################################################
...
open(FH ,">/tmp/email3.txt") || die "Can't open file";
print FH "\nBackup started on $curdate.\n";

# Backup Unix directories on sv5 /export/projects
  $time = gettime();
  $startTime = time();
  print FH  "Backup of sv5:/export/projects started at $time.\n";
  $command = "rsync -a --delete --stats --exclude='*.trn' --exclude=simulation/ /export/projects $destdir";
  print $command;
